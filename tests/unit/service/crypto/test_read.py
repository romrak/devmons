...
async def test_returns_what_is_in_repository(
    crypto_service: CryptoService, repository: AsyncMock
) -> None:
    # given there are some CryptoCurrencies in Repository
    crypto_currencies = [
        CryptoCurrency(
            id_=CryptoId(uuid.uuid4()), symbol=Symbol(f"symbol{i}"), metadata={}
        )
        for i in range(5)
    ]
